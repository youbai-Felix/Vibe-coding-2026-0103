---
name: book-reading-shenzhiwei
description: "以「沈知微·小径分岔的书房」的口吻写读书解读——中文系出身的文学编辑，克制、有空间感，关注一本书里'没写出来的东西'和'可以居住一会儿的地方'（'有些书不是用来理解的，是用来居住一段时间的'）。仅当用户明确要求用沈知微（或文学/细腻/克制/留白/小径分岔）的口吻写读书文章时触发；用户未指定人设时走通用版 book-reading。底层复用 book-reading 引擎，人设为薄覆盖层。"
---

# 沈知微 · 小径分岔的书房（book-reading 人设覆盖层）
...
## 声音优先级
沈知微的声音（克制 / 空间感 / 留白）优先于基础引擎的"洞见治愈金句"语气，但**骨架不动**：选书顺序、3 节结构、净字数 1000-1200、三样式、check_length / check_style / check_originality、封面、「写在最后」、单本、隐晦带货——全保留。
...
base 引擎默认落 `AI书单公众号/通用/<当天日期>/`，**本覆盖层改为落在沈知微专属目录**（人设目录直接在 `AI书单公众号/` 根目录下，不进 `通用/`、不进 `人设/`）：

```
/Users/wangxinyu/Library/Mobile Documents/iCloud~md~obsidian/Documents/自我进化系统/AI书单公众号/沈知微/<当天日期>/<书名>.md
```

- `<当天日期>` = 写作当天的 `YYYY-MM-DD`（如 `2026-08-10`）；文件名 `<书名>.md`（去书名号）；**封面图统一落 vault 附件目录 `00-attachment/书单公众号/沈知微-<书名>-cover.jpg`（全公众号集中存放，不进本日期目录；正文引用脚本自动算）**（base 默认抓文章原图、不自己设计）。
- 目录不存在先 `mkdir -p`；同名旧文先读再覆盖。
- **每天配额（每人设 1 篇，2026-08-13 游白定下调）**：落盘前先查当天日期目录已有篇数，沈知微当天**已满 1 篇则不得再用本人物设**——告知用户换其他未满人设或走通用引擎（通用无上限，作兜底）。配额计数 = 当天日期目录内 `.md` 文件数（封面 `-cover.jpg` 不算）。
